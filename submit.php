...
foreach ($submissions as $s) if (hash_equals($s['ip_hash'], hash('sha256', $ip))) { http_response_code(429); $error='تم إرسال استبيان بالفعل من هذا الاتصال.'; require __DIR__.'/status.php'; exit; }
...
$id=bin2hex(random_bytes(8)); $submissions[]=['id'=>$id,'name'=>$name,'country'=>$country,'phone'=>$phone,'ip_hash'=>hash('sha256',$ip),'created_at'=>date('c'),'answers'=>$clean,'decisions'=>[],'withdrawal'=>'requested','withdrawal_method'=>'vodafone_cash']; save_submissions($submissions); require __DIR__.'/status.php';
